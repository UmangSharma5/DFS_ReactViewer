...
router.get("/thumbnail/:url",async function(req,res){
    try{
        ...
        minioClient.getObject(bucketName, "hv/"+user+"/thumbnail/"+imageName+".png",(err, dataStream) => {
            if (err) {
...
router.get("/pyramid/:url",async function(req,res){
    try{
        ...
        minioClient.getObject(bucketName, `hv/${user}/${baseDir}${level}/${x}_${y}.jpeg`,(err, dataStream) => {
            if (err) {
                console.error('Error getting object from Minio:', err);
                res.status(500).send('Internal Server Error');
                return;
