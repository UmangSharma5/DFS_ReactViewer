...
const file_stats = async (user, bucketName, filename) => {
  let query = `SELECT file_unique_id, file_type, upload_date from FileTypeMap where filename = '${filename}' AND bucket_name = '${bucketName}' AND user_name = '${user}';`;
  return await execSql(query).then(res => {
    return res;
  });
};
const file_uploaded = async (user, bucketName, filename) => {
  let query = `UPDATE FileTypeMap where filename = '${filename}' AND bucket_name = '${bucketName}' AND user_name = '${user}';`;
  return await execSql(query).then(res => {
    return res;
  });
};
